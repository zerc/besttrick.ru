    ...
    initialize: function () {
      ...
      this.on({
        'before:render': function () {
          body.css('overflow', 'hidden').Loader('show');
        },

        'item:before:close': function () {
          body.css('overflow', 'auto').Loader('hide');
        }
      });
    }
  });
...
(function (Form){
  ...
  _.extend(Form.Field.prototype, {
    template:  _.template('\
      <div>\
        <label for="<%= editorId %>">\
          <%= title %><span class="right"><%= help %></span>\
        </label>\
  ...
  _.extend(Form, {
    template: _.template(
      '<form data-fieldsets class="vertical"></form>', 
      null, Form.templateSettings
    )
  });
